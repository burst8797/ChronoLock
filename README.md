...
ChronoLock is a lightweight implementation for generating protected videos before MotionDirector-style Temporal LoRA training. Given an input video and prompt, chronolock optimizes a bounded perturbation that targets temporal adaptation in text-to-video diffusion models while keeping the output in the original video format.

The current release includes:

- `ChronoLock/chronolock.py`: two-stage PGD protection for a single `.mp4` video.
...
chronolock uses a two-stage optimization process:
...
The objective combines a temporal denoising loss, an adversarial temporal difference branch, optional MetaCloak-style transform sampling, and a cross-chunk boundary term. The implementation is intentionally compact so experiments can be reproduced and modified directly from `ChronoLock/chronolock.py`.
...
Clean-training results are generated by MotionDirector models trained on clean videos. Protected-training results are generated by MotionDirector models trained on chronolock-protected videos.
...
conda create -n chronolock python=3.11 -y
conda activate chronolock
...
The examples below use Linux shell syntax. On Windows PowerShell, set GPUs first with `$env:CUDA_VISIBLE_DEVICES="0,1"` and then run the `python chronolock.py ...` command without the `CUDA_VISIBLE_DEVICES=0,1` prefix.

### Single Video

```bash
CUDA_VISIBLE_DEVICES=0,1 python chronolock.py \
...
For each processed video, chronolock writes:
